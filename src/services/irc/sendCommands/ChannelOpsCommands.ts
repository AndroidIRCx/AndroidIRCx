...
export const handleBAN: SendMessageHandler = (ctx, args, target) => {
  ...
  if (args.length >= 1) {
    ...
    if (banChannel) {
      ...
      ctx.addMessage({
        type: 'notice',
        text: t('*** Banning {nick} from {channel}', {
          nick: banNick,
          channel: banChannel,
        }),
        timestamp: Date.now(),
...
export const handleUNBAN: SendMessageHandler = (ctx, args, target) => {
  ...
  if (args.length >= 1) {
    ...
    if (unbanChannel) {
      ...
      ctx.addMessage({
        type: 'notice',
        text: t('*** Unbanning {mask} from {channel}', {
          mask: unbanMask,
      ...
      ctx.addMessage({
        type: 'error',
        text: t('Usage: /unban <nick|mask> [channel]'),
        timestamp: Date.now(),
      });
    }
  } else {
...
export const handleKICKBAN: SendMessageHandler = (ctx, args, target) => {
  ...
  if (args.length >= 1) {
    const kbNick = args[0];
    const kbChannel =
      args.length > 1 && (args[1].startsWith('#') || args[1].startsWith('&'))
        ? args[1]
        : target.startsWith('#') || target.startsWith('&')
          ? target
          : args[1] || '';
    ...
    if (kbChannel && (kbChannel.startsWith('#') || kbChannel.startsWith('&'))) {
      ...
      ctx.sendCommand(
        `KICK ${kbChannel} ${kbNick}${kbReason ? ` :${kbReason}` : ''}`,
      );
      ctx.addMessage({
        type: 'notice',
        text: t('*** Kicking and banning {nick} from {channel}', {
          nick: kbNick,
          channel: kbChannel,
        }),
        timestamp: Date.now(),
      ...
      ctx.addMessage({
        type: 'error',
        text: t('Usage: /kickban <nick> [channel] [reason]'),
        timestamp: Date.now(),
      });
    }
  } else {
...
export const handleINVITE: SendMessageHandler = (ctx, args, target) => {
  ...
  if (args.length >= 1) {
    ...
    if (inviteChannel) {
      ...
      ctx.addMessage({
        type: 'notice',
        text: t('*** Invited {nick} to {channel}', {
          nick: inviteNick,
      ...
      ctx.addMessage({
        type: 'error',
        text: t('Usage: /invite <nick> [channel]'),
        timestamp: Date.now(),
      });
    }
  } else {

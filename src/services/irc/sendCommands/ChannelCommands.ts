...
export const handleCNOTICE: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length >= 3) {
    ...
    ctx.addMessage({
      type: 'notice',
      channel: cnoticeChannel,
      from: ctx.getCurrentNick(),
      text: `-> ${cnoticeNick}: ${cnoticeMessage}`,
      timestamp: Date.now(),
      status: 'sent',
    });
  } else {
    ctx.addMessage({
      type: 'error',
      text: t('Usage: /cnotice <nick> <channel> <message>'),
      timestamp: Date.now(),
    });
  }
};
...
export const handleCPRIVMSG: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length >= 3) {
    ...
    ctx.sendRaw(
      `CPRIVMSG ${cprivmsgNick} ${cprivmsgChannel} :${cprivmsgMessage}`,
    );
    ctx.addMessage({
      type: 'message',
      channel: cprivmsgChannel,
      from: ctx.getCurrentNick(),
      text: `-> ${cprivmsgNick}: ${cprivmsgMessage}`,
      timestamp: Date.now(),
      status: 'sent',
    });
  } else {
    ctx.addMessage({
      type: 'error',
      text: t('Usage: /cprivmsg <nick> <channel> <message>'),
      timestamp: Date.now(),
    });
  }
};
...
export const handleCHAT: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length >= 2) {
    ...
    ctx.addMessage({
      type: 'error',
      text: t('Usage: /chat <service> <message>'),
      timestamp: Date.now(),
    });
  }
};
...
export const handleANICK: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length > 0) {
    ...
    ctx.addMessage({
      type: 'notice',
      text: t('*** Alternate nickname set to: {nick}', { nick: args[0] }),
      timestamp: Date.now(),
    });
...
export const handleAJINVITE: SendMessageHandler = (ctx, args) => {
  ...
  ctx.addMessage({
    type: 'notice',
    text: t('*** Auto-join on invite: {state}', { state: isOn ? 'ON' : 'OFF' }),
    timestamp: Date.now(),
  });
};

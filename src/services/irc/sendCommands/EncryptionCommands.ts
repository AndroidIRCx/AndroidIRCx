...
export const handleSHAREKEY: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length > 0) {
    ...
    encryptedDMService
      ...
      .then((bundle: any) => {
        ctx.sendRaw(
          `PRIVMSG ${keyTarget} :!enc-offer ${JSON.stringify(bundle)}`,
        );
        ctx.addMessage({
          type: 'system',
          channel: keyTarget,
      ...
      .catch((e: any) => {
        ctx.addMessage({
          type: 'error',
          text: t('*** Failed to share encryption key: {message}', {
            message: e.message,
          }),
          timestamp: Date.now(),
        });
...
export const handleENCMSG: SendMessageHandler = (ctx, args) => {
  ...
  if (args.length >= 2) {
    ...
    encryptedDMService
      .encryptForNetwork(encPlaintext, network, encTarget)
      .then((payload: any) => {
        ctx.sendRaw(
          `PRIVMSG ${encTarget} :!enc-msg ${JSON.stringify(payload)}`,
        );
        ctx.addMessage({
          type: 'message',
          channel: encTarget,
      ...
      .catch((e: any) => {
        ctx.addMessage({
          type: 'error',
          text: t(
            '*** Encrypted send failed ({message}). Use "Request Encryption Key" from the user menu.',
            { message: e.message },
          ),
          timestamp: Date.now(),
        });
...
export const handleENC: SendMessageHandler = ctx => {
  ...
  ctx.addMessage({
    type: 'notice',
    text: t(
      'Tips: keys must be exchanged first; use /requestkey if not paired.',
    ),
    timestamp: Date.now(),
  });
};
...
export const handleCHANKEY: SendMessageHandler = (ctx, args, target) => {
  ...
  if (args.length === 0) {
    ctx.addMessage({
      type: 'error',
      text: t(
        'Usage: /chankey <generate|share|request|remove|send|help> [args]',
      ),
      timestamp: Date.now(),
    });
    return;
  ...
  switch (chankeyAction) {
    ...
    case 'generate':
      ...
      channelEncryptionService
        .generateChannelKey(target, ctx.getNetworkName())
        .then(() => {
          ctx.addMessage({
            type: 'notice',
            text: t(
              '*** Channel encryption key generated for {channel}. Use /chankey share <nick> to share with others.',
              { channel: target },
            ),
            timestamp: Date.now(),
          });
        ...
        .catch((e: any) => {
          ctx.addMessage({
            type: 'error',
            text: t('*** Failed to generate channel key: {message}', {
              message: e.message,
            }),
            timestamp: Date.now(),
          });
    ...
    case 'send':
      ...
      }
      {
        ...
        channelEncryptionService
          ...
          .then((payload: any) => {
            ctx.sendRaw(
              `PRIVMSG ${target} :!chanenc-msg ${JSON.stringify(payload)}`,
            );
            ctx.addMessage({
              type: 'message',
              channel: target,
              ...
              text: t('*** Channel encryption send failed: {message}', {
                message:
                  e.message === 'no channel key'
                    ? t(
                        'Missing channel key. Use /chankey generate and share first.',
                      )
                    : e.message,
              }),
              timestamp: Date.now(),
    ...
    case 'share':
      ...
      }
      {
        ...
        channelEncryptionService
          .exportChannelKey(target, ctx.getNetworkName())
          .then((keyData: any) => {
          ...
          .catch((e: any) => {
            ctx.addMessage({
              type: 'error',
              text: t(
                '*** Failed to share channel key: {message}. Generate a key first with /chankey generate',
                { message: e.message },
              ),
              timestamp: Date.now(),
            });
    ...
    case 'request':
      ...
      }
      {
        ...
        ctx.sendRaw(
          `PRIVMSG ${requestTarget} :${t('Please share the channel key for {channel} with /chankey share {nick}', { channel: target, nick: ctx.getCurrentNick() })}`,
        );
        ctx.addMessage({
          type: 'notice',
          text: t('*** Channel key requested from {nick} for {channel}', {
            nick: requestTarget,
            channel: target,
          }),
          timestamp: Date.now(),
    ...
    case 'remove':
      ...
      channelEncryptionService
        .removeChannelKey(target, ctx.getNetworkName())
        .then(() => {
          ctx.addMessage({
            type: 'notice',
            text: t('*** Channel encryption key removed for {channel}', {
              channel: target,
            }),
            timestamp: Date.now(),
          });
        ...
        .catch((e: any) => {
          ctx.addMessage({
            type: 'error',
            text: t('*** Failed to remove channel key: {message}', {
              message: e.message,
            }),
            timestamp: Date.now(),
          });

...
function resolveChannelIdentifier(
  isChannel: boolean,
  target: string,
  fromNick: string,
  currentNick: string,
  ...
  return isChannel
    ? target
    : fromNick.toLowerCase() === currentNick.toLowerCase()
      ? target
      : fromNick;
}
...
function handleEncryptionProtocol(
  ...
  if (msgText.startsWith('!enc-key ')) {
    encryptedDMService.handleIncomingBundleForNetwork(
      network,
      fromNick,
      msgText.substring('!enc-key '.length),
    );
    return true;
  }
  ...
  if (msgText.startsWith('!enc-offer ')) {
    encryptedDMService.handleKeyOfferForNetwork(
      network,
      fromNick,
      msgText.substring('!enc-offer '.length),
    );
    return true;
  }
  ...
  if (msgText.startsWith('!enc-accept ')) {
    encryptedDMService
      .handleKeyAcceptanceForNetwork(
        network,
        fromNick,
        msgText.substring('!enc-accept '.length),
      )
      .then((result: any) => {
        if (result.status === 'stored') {
          ctx.addMessage({
            type: 'notice',
            text: t(
              '*** {nick} accepted your encryption key. Encrypted chat enabled.',
              { nick: fromNick },
            ),
            timestamp: Date.now(),
          });
          ...
          ctx.addMessage({
            type: 'notice',
            text: t(
              '*** {nick} sent a different encryption key. Review and accept the new key to continue encrypted chat.',
              { nick: fromNick },
            ),
            timestamp: Date.now(),
          });
      ...
      .catch((e: any) =>
        console.warn('EncryptedDMService: failed to handle acceptance', e),
      );
    return true;
  }
  ...
  if (msgText === '!enc-reject') {
    ctx.addMessage({
      type: 'notice',
      text: t('*** {nick} rejected your encryption key offer.', {
        nick: fromNick,
      }),
      timestamp: Date.now(),
    });
  ...
  if (msgText === '!enc-req') {
    encryptedDMService
      ...
      .then((bundle: any) =>
        ctx.sendRaw(
          `PRIVMSG ${fromNick} :!enc-offer ${JSON.stringify(bundle)}`,
        ),
      )
      .catch((e: any) =>
        console.warn('EncryptedDMService: failed to respond to enc-req', e),
      );
    return true;
  }
    ...
    channelEncryptionService
      ...
      .catch((e: any) => {
        ctx.addMessage({
          ...
          text: t('🔒 {message}', {
            message:
              e.message === 'no channel key'
                ? t(
                    'Missing channel key. Use /chankey request <nick> to get it.',
                  )
                : t(
                    'Decryption failed. If this was sent from v1.6.3+, please update to v1.6.3 or newer.',
                  ),
          }),
          timestamp,
        });
  ...
  if (msgText.startsWith('!chanenc-key ')) {
    ...
    channelEncryptionService
      .importChannelKey(keyData, network)
      .then((imported: any) => {
        ctx.addMessage({
          type: 'notice',
          text: t('*** Received channel key for {channel} from {nick}', {
      ...
      .catch((e: any) => {
        ctx.addMessage({
          type: 'error',
          text: t('*** Failed to import channel key: {message}', {
            message: e,
          }),
          timestamp: Date.now(),
        });
    ...
    encryptedDMService
      ...
      .catch(() => {
        ctx.addMessage({
          ...
          channel: channelIdentifier,
          text: t(
            'Encrypted message from {nick} could not be decrypted. If they are on v1.6.3+, please update to v1.6.3 or newer.',
            { nick: fromNick },
          ),
          timestamp,
        });
...
function handleWebRTCCallProtocol(
  ...
  if (fromNick.toLowerCase() === ctx.getCurrentNick().toLowerCase()) {
    return (
      msgText.startsWith('!webrtc ') || msgText.startsWith('!webrtc-chunk ')
    );
  }
...
export const handlePRIVMSG: CommandHandler = (
  ctx,
  prefix,
  params,
  timestamp,
  ...
  if (
    !isChannel &&
    fromNick === ctx.getCurrentNick() &&
    target === ctx.getCurrentNick()
  ) {
    return;
  }
  ...
  if (
    !ctx.isUserProtected(fromNick, username, hostname, network) &&
  ...
  const protectionContext = ctx.getProtectionTabContext(
    target,
    fromNick,
    isChannel,
  );
  ...
  const protectionDecision = ctx.isUserProtected(
    fromNick,
    username,
    hostname,
    network,
  ...
  if (protectionDecision) {
    ctx.handleProtectionBlock(
      protectionDecision.kind,
      fromNick,
      username,
      hostname,
      isChannel ? target : null,
    );
    return;
  }
  ...
  const channelIdentifier = resolveChannelIdentifier(
    isChannel,
    target,
    fromNick,
    ctx.getCurrentNick(),

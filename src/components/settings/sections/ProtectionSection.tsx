...
export const ProtectionSection: React.FC<ProtectionSectionProps> = ({
  ...
  const [spamPmMode, setSpamPmMode] = useState<'when_open' | 'always'>(
    'when_open',
  );
  ...
  const [protIrcopAction, setProtIrcopAction] = useState<
    'none' | 'ban' | 'kill' | 'kline' | 'gline'
  >('none');
  const [protIrcopReason, setProtIrcopReason] = useState(
    'Auto protection: spam/flood',
  );
  ...
  const [confirmBeforeKickBan, setConfirmBeforeKickBan] =
  ...
  useEffect(() => {
    const load = async () => {
      setSpamPmMode(
        await settingsService.getSetting('spamPmMode', 'when_open'),
      );
      setSpamPmKeywords(
      ...
      setSpamChannelEnabled(
        await settingsService.getSetting('spamChannelEnabled', false),
      );
      setSpamNoSpamOnQuits(
        await settingsService.getSetting('spamNoSpamOnQuits', false),
      );
      setSpamLoggingEnabled(
        await settingsService.getSetting('spamLoggingEnabled', false),
      );

      setProtCtcpFlood(
        await settingsService.getSetting('protCtcpFlood', false),
      );
      setProtTextFlood(
        await settingsService.getSetting('protTextFlood', false),
      );
      setProtDccFlood(await settingsService.getSetting('protDccFlood', false));
      setProtQueryFlood(
        await settingsService.getSetting('protQueryFlood', false),
      );
      setProtDosAttacks(
        await settingsService.getSetting('protDosAttacks', false),
      );
      setProtAntiDeopEnabled(
        await settingsService.getSetting('protAntiDeopEnabled', false),
      );
      setProtAntiDeopUseChanserv(
        await settingsService.getSetting('protAntiDeopUseChanserv', false),
      );
      setProtExcludeTokens(
        await settingsService.getSetting('protExcludeTokens', ''),
      );
      setProtEnforceSilence(
        await settingsService.getSetting('protEnforceSilence', false),
      );
      setProtBlockTsunamis(
        await settingsService.getSetting('protBlockTsunamis', false),
      );
      setProtTextFloodNet(
        await settingsService.getSetting('protTextFloodNet', false),
      );
      setProtIrcopAction(
        await settingsService.getSetting('protIrcopAction', 'none'),
      );
      setProtIrcopReason(
        await settingsService.getSetting(
          'protIrcopReason',
      ...
      setProtIrcopDuration(
        await settingsService.getSetting('protIrcopDuration', '1h'),
      );
      ...
      setDefaultBanType(
        await settingsService.getSetting(
          'defaultBanType',
          NEW_FEATURE_DEFAULTS.defaultBanType,
        ),
      );
      setPredefinedKickReasons(
        await settingsService.getSetting(
          'predefinedKickReasons',
          NEW_FEATURE_DEFAULTS.predefinedKickReasons,
        ),
      );
      setShowBanMaskPreview(
        await settingsService.getSetting(
          'showBanMaskPreview',
          NEW_FEATURE_DEFAULTS.showBanMaskPreview,
        ),
      );
      setConfirmBeforeKickBan(
        await settingsService.getSetting(
          'confirmBeforeKickBan',
          NEW_FEATURE_DEFAULTS.confirmBeforeKickBan,
        ),
      );
    };
    load();
  ...
  const spamItems: SettingItemType[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description: t('{count} patterns', {
          count: spamPmKeywords.length,
          _tags: tags,
        }),
        type: 'button',
  ...
  const floodItems: SettingItemType[] = useMemo(
    () => [
      ...
      // Ban settings
      {
        ...
        description: t('Select the default ban mask type (0-11)', {
          _tags: tags,
        }),
        type: 'button',
      ...
      },
      {
        ...
        description: t('{count} reasons', {
          count: predefinedKickReasons.length,
          _tags: tags,
        }),
        type: 'button',
      ...
      },
      {
        ...
        description: t('Show reason dialog for all kick/ban actions', {
          _tags: tags,
        }),
        type: 'switch',
  ...
  return (
    <View>
      <View style={stylesLocal.tabRow}>
        <TouchableOpacity
          style={[
            ...
            activeTab === 'spam' && stylesLocal.tabButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              stylesLocal.tabText,
              activeTab === 'spam' && stylesLocal.tabTextActive,
            ]}
          >
        ...
        <TouchableOpacity
          style={[
            stylesLocal.tabButton,
            activeTab === 'flood' && stylesLocal.tabButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              stylesLocal.tabText,
              activeTab === 'flood' && stylesLocal.tabTextActive,
            ]}
          >
          ...
          <View style={stylesLocal.modalContent}>
            ...
            <View style={stylesLocal.modalRow}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (
                    editingKeywordIndex !== null &&
                    editingKeywordIndex >= 0 &&
                    editingKeywordIndex < updated.length
                  ) {
            ...
            <ScrollView style={{ maxHeight: 260 }}>
              {spamPmKeywords.map((keyword, idx) => (
                <View key={`${keyword}-${idx}`} style={stylesLocal.presetItem}>
                  <View style={stylesLocal.presetRow}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={async () => {
                        const updated = spamPmKeywords.filter(
                          (_, i) => i !== idx,
                        );
                        setSpamPmKeywords(updated);
      ...
      <Modal
        visible={showSpamModeModal}
        transparent
        animationType="fade"
        onRequestClose={() => setShowSpamModeModal(false)}
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            <Text style={stylesLocal.modalTitle}>
      ...
      <Modal
        visible={showSpamLogModal}
        transparent
        animationType="fade"
        onRequestClose={() => setShowSpamLogModal(false)}
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            <Text style={stylesLocal.modalTitle}>
      ...
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            ...
            <Text style={[stylesLocal.presetText, { marginBottom: 12 }]}>
              {t('Are you sure you want to clear the spam log?', {
                _tags: tags,
              })}
            </Text>
            <View style={stylesLocal.modalButtonRow}>
      ...
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            ...
            <Text style={[stylesLocal.presetText, { marginBottom: 12 }]}>
              {t(
                'Protections will ignore spam/flood users and can trigger IRCop actions when enabled.',
                { _tags: tags },
              )}
            </Text>
            <View style={stylesLocal.modalButtonRow}>
      ...
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            <Text style={stylesLocal.modalTitle}>
              {t('Select Default Ban Type', { _tags: tags })}
            </Text>
            <Text style={[stylesLocal.presetText, { marginBottom: 12 }]}>
              ...
              {[
                {
                  id: 0,
                  label: '0 - *!user@host',
                  desc: t('Ban by user@host', { _tags: tags }),
                },
                {
                  id: 1,
                  label: '1 - *!*user@host',
                ...
                },
                {
                  id: 2,
                  label: '2 - *!*@host',
                  desc: t('Ban by host only', { _tags: tags }),
                },
                {
                  id: 3,
                  label: '3 - *!*user@*.host',
                  desc: t('Ban by *user@*.domain', { _tags: tags }),
                },
                {
                  id: 4,
                  label: '4 - *!*@*.host',
                  desc: t('Ban by *.domain only', { _tags: tags }),
                },
                {
                  id: 5,
                  label: '5 - nick!user@host',
                  desc: t('Ban exact nick!user@host', { _tags: tags }),
                },
                {
                  id: 6,
                  label: '6 - nick!*user@host',
                  desc: t('Ban nick with *user@host', { _tags: tags }),
                },
                {
                  id: 7,
                  label: '7 - nick!*@host',
                  desc: t('Ban nick with any user@host', { _tags: tags }),
                },
                {
                  id: 8,
                  label: '8 - nick!*user@*.host',
                  desc: t('Ban nick with *user@*.domain', { _tags: tags }),
                },
                {
                  id: 9,
                  label: '9 - nick!*@*.host',
                  desc: t('Ban nick with *.domain', { _tags: tags }),
                },
                {
                  id: 10,
                  label: '10 - nick!*@*',
                  desc: t('Ban by nick only (any ident@host)', { _tags: tags }),
                },
                {
                  id: 11,
                  label: '11 - *!ident@*',
                  desc: t('Ban by ident only (any nick@host)', { _tags: tags }),
                },
              ].map(bt => (
                <TouchableOpacity
                  key={bt.id}
                ...
                >
                  <Text
                    style={[
                      stylesLocal.presetText,
                      { fontWeight: defaultBanType === bt.id ? '600' : '400' },
                    ]}
                  >
                    {bt.label}
                  </Text>
      ...
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            <Text style={stylesLocal.modalTitle}>
              {t('Predefined Kick/Ban Reasons', { _tags: tags })}
            </Text>
            ...
            <View style={stylesLocal.modalRow}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (
                    editingReasonIndex !== null &&
                    editingReasonIndex >= 0 &&
                    editingReasonIndex < updated.length
                  ) {
                ...
                <View key={`${reason}-${idx}`} style={stylesLocal.presetItem}>
                  <View style={stylesLocal.presetRow}>
                    ...
                    <TouchableOpacity
                      style={stylesLocal.removeButton}
                      onPress={async () => {
                        const updated = predefinedKickReasons.filter(
                          (_, i) => i !== idx,
                        );
                        setPredefinedKickReasons(updated);
            ...
            <View style={stylesLocal.modalButtonRow}>
              ...
              >
                <Text style={stylesLocal.modalButtonTextSecondary}>
                  {t('Reset to Defaults', { _tags: tags })}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[

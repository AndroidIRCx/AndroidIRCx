...
export const MediaSection: React.FC<MediaSectionProps> = ({
  ...
  const [videoQuality, setVideoQuality] = useState<
    '4k' | '1080p' | '720p' | '480p'
  >('1080p');
  const [callVideoQuality, setCallVideoQuality] = useState<
  ...
  const [callNicklistCallActionsEnabled, setCallNicklistCallActionsEnabled] =
    useState(false);
  ...
  const [callOverlayOnlyOnActiveQuery, setCallOverlayOnlyOnActiveQuery] =
    useState(false);
    ...
    const loadSettings = async () => {
      try {
        ...
        const autoDownloadSetting =
          await mediaSettingsService.getAutoDownload();
        ...
        const capabilityProfile =
          callMediaProfileService.getCapabilityProfile();
        setRelayEnabled(capabilityProfile.relayEnabled);
        const safeCallQuality =
          capabilityProfile.allowedVideoQualities.includes(callQuality as any)
            ? callQuality
        ...
        const forceRelayOnly =
          await mediaSettingsService.getCallForceRelayOnly();
        setCallForceRelayOnly(forceRelayOnly);

        const nicklistCallActionsEnabled =
          await mediaSettingsService.getCallNicklistCallActionsEnabled();
        ...
        const showCallNotif = await settingsService.getSetting(
          'showCallNotification',
          true,
        );
        setShowCallNotification(Boolean(showCallNotif));

        const scopedOverlay = await settingsService.getSetting(
  ...
  const handleClearCache = useCallback(async () => {
    Alert.alert(
      ...
      t(
        'Are you sure you want to clear all cached media? This will free up storage space but media will need to be downloaded again.',
        { _tags: tags },
      ),
      [
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                ...
                t('Media cache has been cleared successfully.', {
                  _tags: tags,
                }),
              );
            } catch (error) {
  ...
  const isValidTurnUrl = (value: string): boolean =>
  ...
  const sectionData: SettingItemType[] = useMemo(() => {
    ...
    const hasTurnCredentials =
      callTurnUsername.trim().length > 0 && callTurnCredential.length > 0;
    const callQualityItems = capabilityProfile.allowedVideoQualities.map(
      quality => ({
    ...
    const items: SettingItemType[] = [
      {
        ...
        description: mediaEnabled
          ? t(
              'Media sharing is enabled. Attachment button (📎) appears on encrypted conversations.',
              { _tags: tags },
            )
          : t('Media sharing is disabled. Attachment button will not appear.', {
              _tags: tags,
            }),
        type: 'switch',
        value: mediaEnabled,
      ...
      },
      {
        ...
        description: t(
          'Media sharing works only in encrypted conversations (DMs with key exchange or encrypted channels). All media is automatically encrypted using existing E2E keys.',
          { _tags: tags },
        ),
        type: 'button' as const,
        disabled: true,
      ...
      },
      {
        ...
        description: t(
          'Display 🔒 icon on media thumbnails to indicate encryption',
          { _tags: tags },
        ),
        type: 'switch',
      ...
      },
      {
        ...
        description: t('Automatically download media when received', {
          _tags: tags,
        }),
        type: 'switch',
        value: autoDownload,
      ...
      },
      {
        ...
        description: t('Only download media when connected to WiFi', {
          _tags: tags,
        }),
        type: 'switch',
        value: wifiOnly,
        ...
        submenuItems: [
          {
            ...
            title: '50 MB',
            type: 'button' as const,
            onPress: () => setMaxCacheSize(50 * 1024 * 1024),
          },
          {
            id: 'cache-100mb',
            title: '100 MB',
            type: 'button' as const,
            onPress: () => setMaxCacheSize(100 * 1024 * 1024),
          },
          {
            id: 'cache-250mb',
            title: '250 MB',
            type: 'button' as const,
            onPress: () => setMaxCacheSize(250 * 1024 * 1024),
          },
          {
            id: 'cache-500mb',
            title: '500 MB',
            type: 'button' as const,
            onPress: () => setMaxCacheSize(500 * 1024 * 1024),
          },
          {
            id: 'cache-1gb',
            title: '1 GB',
            type: 'button' as const,
            onPress: () => setMaxCacheSize(1024 * 1024 * 1024),
          },
        ],
      },
      {
        ...
        description: t('Current: {quality}', {
          quality: mediaQualityLabel,
          _tags: tags,
        }),
        type: 'submenu',
        disabled: !mediaEnabled,
        ...
        submenuItems: [
          {
            id: 'quality-original',
            title: t('Original', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setMediaQuality('original'),
          },
          {
            id: 'quality-high',
            title: t('High', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setMediaQuality('high'),
          },
          {
            id: 'quality-medium',
            title: t('Medium', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setMediaQuality('medium'),
          },
          {
            id: 'quality-low',
            title: t('Low', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setMediaQuality('low'),
          },
        ],
      },
      {
        ...
        description: relayEnabled
          ? t(
              'Current: {quality} • Privacy Relay unlocks HD and TURN fallback',
              {
          ...
          : t(
              'Current: {quality} • Free calls stay direct, default to 480p, and can be raised to 720p',
              {
                quality: callVideoQualityLabel,
                _tags: tags,
              },
      ...
      },
      {
        ...
        description: relayEnabled
          ? t(
              'Current fallback list: {count} STUN endpoints. Premium calls still use TURN when needed.',
              {
          ...
          : t(
              'Current fallback list: {count} STUN endpoints for direct free calls. Ordered list is used for fallback.',
              {
                count: callStunServers.length,
                _tags: tags,
              },
      ...
      },
      {
        ...
        description: callTurnEnabled
          ? t('Enabled: {count} TURN URL(s), credentials {credentialsState}.', {
              ...
              credentialsState: hasTurnCredentials
                ? t('set', { _tags: tags })
                : t('missing', { _tags: tags }),
              _tags: tags,
            })
          : t(
              'Disabled. Configure your own TURN relay if you already have one.',
              { _tags: tags },
            ),
        type: 'submenu',
        disabled: !mediaEnabled,
      ...
      },
      {
        ...
        description: t(
          'If enabled, WebRTC uses relay candidates only when relay is available (Privacy Relay or external TURN).',
          { _tags: tags },
        ),
        type: 'switch',
      ...
      },
      {
        ...
        description: callNicklistCallActionsEnabled
          ? t(
              'Audio and Video Call actions are visible in user context menu.',
              { _tags: tags },
            )
          : t('Audio and Video Call actions are hidden in user context menu.', {
              _tags: tags,
            }),
        type: 'switch',
      ...
      },
      {
        ...
        description: t(
          'Keeps an Android notification visible during active audio and video calls so you can jump back quickly.',
          { _tags: tags },
        ),
        type: 'switch',
      ...
      },
      {
        ...
        description: callOverlayOnlyOnActiveQuery
          ? t(
              'Minimized call overlay appears only when you are on the matching query tab. This is optional and off by default.',
              { _tags: tags },
            )
          : t(
              'Default behavior: minimized call overlay stays visible everywhere in the app.',
              { _tags: tags },
            ),
        type: 'switch',
      ...
      },
      {
        ...
        description: t('Current: {duration} seconds', {
        ...
        submenuItems: [
          {
            id: 'voice-60s',
            title: t('1 minute', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setVoiceMaxDuration(60),
          },
          {
            id: 'voice-180s',
            title: t('3 minutes', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setVoiceMaxDuration(180),
          },
          {
            id: 'voice-300s',
            title: t('5 minutes', { _tags: tags }),
            type: 'button' as const,
            onPress: () => setVoiceMaxDuration(300),
          },
        ],
      },
      {
        ...
        description: t('Free up storage by clearing cached media files', {
          _tags: tags,
        }),
        type: 'button',
  ...
  useEffect(() => {
    ...
    const saveSettings = async () => {
      ...
      await mediaSettingsService.setCallNicklistCallActionsEnabled(
        callNicklistCallActionsEnabled,
      );
        ...
        <View style={modalStyles.serverRowActions}>
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              type === 'stun'
                ? setCallStunServers(next)
                : setCallTurnServers(next);
            }}
            disabled={index === 0}
            style={[
              modalStyles.serverActionButton,
              index === 0 && modalStyles.serverActionButtonDisabled,
            ]}
          >
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              index === servers.length - 1 &&
                modalStyles.serverActionButtonDisabled,
            ]}
          >
          ...
          >
            <Text style={modalStyles.serverActionTextDanger}>
              {t('Remove', { _tags: tags })}
            </Text>
          </TouchableOpacity>
        </View>
                ...
                <View style={modalStyles.submenuItem}>
                  <View style={modalStyles.submenuItemContent}>
                    <Text style={modalStyles.submenuItemText}>
                      {t('STUN Servers', { _tags: tags })}
                    </Text>
                    <Text style={modalStyles.submenuItemDescription}>
                      {t(
                        'Add multiple STUN URLs. Order is used as fallback priority.',
                        { _tags: tags },
                      )}
                    </Text>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          if (!isValidStunUrl(nextValue)) {
                            setCallStunDraftError(
                              t('STUN must start with stun:', { _tags: tags }),
                            );
                            return;
                          }
                          setCallStunServers(current =>
                            addUniqueServer(current, nextValue),
                          );
                          setCallStunDraft('');
                          setCallStunDraftError('');
                        }}
                      >
                        <Text style={modalStyles.addServerButtonText}>
                          {t('Add', { _tags: tags })}
                        </Text>
                      </TouchableOpacity>
                    </View>
                    {Boolean(callStunDraftError) && (
                      <Text style={modalStyles.serverInputErrorText}>
                        {callStunDraftError}
                      </Text>
                    )}
                  </View>
              ...
              {isTurnSubmenu && (
                <>
                  <View style={modalStyles.submenuItem}>
                    <View style={modalStyles.submenuItemContent}>
                      <Text style={modalStyles.submenuItemText}>
                        {t('Enable External TURN', { _tags: tags })}
                      </Text>
                      <Text style={modalStyles.submenuItemDescription}>
                        {t(
                          'Use your own TURN server for call relay fallback.',
                          { _tags: tags },
                        )}
                      </Text>
                    </View>
                    <Switch
                      ...
                      onValueChange={value =>
                        setCallTurnEnabled(Boolean(value))
                      }
                    />
                  </View>
                  <View style={modalStyles.submenuItem}>
                    <View style={modalStyles.submenuItemContent}>
                      <Text style={modalStyles.submenuItemText}>
                        {t('TURN Servers', { _tags: tags })}
                      </Text>
                      <Text style={modalStyles.submenuItemDescription}>
                        {t(
                          'Add multiple TURN URLs. Order is used as fallback priority.',
                          { _tags: tags },
                        )}
                      </Text>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            if (!isValidTurnUrl(nextValue)) {
                              setCallTurnDraftError(
                                t('TURN must start with turn: or turns:', {
                                  _tags: tags,
                                }),
                              );
                              return;
                            }
                            setCallTurnServers(current =>
                              addUniqueServer(current, nextValue),
                            );
                            setCallTurnDraft('');
                            setCallTurnDraftError('');
                  ...
                  return (
                    ...
                    >
                      <View style={modalStyles.submenuItemContent}>
                        <Text
                          style={[
                            modalStyles.submenuItemText,
                            subItem.disabled && modalStyles.disabledText,
                          ]}
                        >
                          {subItem.title}
                        </Text>
                        {subItem.description && (
                          <Text
                            style={[
                              modalStyles.submenuItemDescription,
                              subItem.disabled && modalStyles.disabledText,
                            ]}
                          >
                            {subItem.description}

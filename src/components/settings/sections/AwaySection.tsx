...
export const AwaySection: React.FC<AwaySectionProps> = ({
  ...
  const [awayAutoAnswerMessages, setAwayAutoAnswerMessages] = useState<
    string[]
  >([]);
  ...
  const [colorInsertTarget, setColorInsertTarget] = useState<
    'away' | 'auto' | null
  >(null);
  const [editingPresetIndex, setEditingPresetIndex] = useState<number | null>(
    null,
  );
  const [editingAutoAnswerIndex, setEditingAutoAnswerIndex] = useState<
    number | null
  >(null);

  useEffect(() => {
    const load = async () => {
      const storedNickPattern = await settingsService.getSetting(
        'awayNickPattern',
        '<me>^away',
      );
      const normalizedNickPattern =
        storedNickPattern === '(<me>^away)' ? '<me>^away' : storedNickPattern;
      setAwayNickPattern(normalizedNickPattern);
      if (normalizedNickPattern !== storedNickPattern) {
        await settingsService.setSetting(
          'awayNickPattern',
          normalizedNickPattern,
        );
      }
      setAwayNotifyMentions(
        await settingsService.getSetting('awayNotifyMentions', false),
      );
      setAwayDeopOnChannels(
        await settingsService.getSetting('awayDeopOnChannels', false),
      );
      setAwayAnnounceEnabled(
        await settingsService.getSetting('awayAnnounceEnabled', false),
      );
      setAwayAutoAnswerEnabled(
        await settingsService.getSetting('awayAutoAnswerEnabled', false),
      );
      setAwayDisableSounds(
        await settingsService.getSetting('awayDisableSounds', false),
      );
      setAwayMultiServer(
        await settingsService.getSetting('awayMultiServer', false),
      );
      setAwayPresets(await settingsService.getSetting('awayPresets', []));
      setAwaySelectedPreset(
        await settingsService.getSetting('awaySelectedPreset', ''),
      );
      setAwayDefaultReason(
        await settingsService.getSetting('awayDefaultReason', ''),
      );

      setAwayAutoAnswerMessage(
        await settingsService.getSetting('awayAutoAnswerMessage', 'Away'),
      );
      setAwayAutoAnswerMessages(
        await settingsService.getSetting('awayAutoAnswerMessages', []),
      );
      setAwayAnnounceEveryMin(
        String(await settingsService.getSetting('awayAnnounceEveryMin', 5)),
      );
      setAwayAnnounceOnlyOn(
        await settingsService.getSetting('awayAnnounceOnlyOn', ''),
      );
      setAwayAnnounceExcludeOn(
        await settingsService.getSetting('awayAnnounceExcludeOn', ''),
      );
      setAwayTextAway(await settingsService.getSetting('awayTextAway', 'away'));
      setAwayTextReturn(
        await settingsService.getSetting('awayTextReturn', 'back'),
      );
      setAutoAwayEnabled(
        await settingsService.getSetting('autoAwayEnabled', false),
      );
      setAutoAwayMinutes(
        String(await settingsService.getSetting('autoAwayMinutes', 0)),
      );
      setAutoAwayReason(await settingsService.getSetting('autoAwayReason', ''));
      setMinimizeToTray(
        await settingsService.getSetting('minimizeToTray', false),
      );
    };
    load();
  }, []);
  ...
  const awayItems: SettingItemType[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description: t('{count} presets', {
          count: awayPresets.length,
          _tags: tags,
        }),
        type: 'button',
  ...
  const optionItems: SettingItemType[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description: t('{count} messages', {
          count: awayAutoAnswerMessages.length,
          _tags: tags,
        }),
        type: 'button',
    ...
    <View>
      <View style={stylesLocal.tabRow}>
        <TouchableOpacity
          style={[
            ...
            activeTab === 'system' && stylesLocal.tabButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              stylesLocal.tabText,
              activeTab === 'system' && stylesLocal.tabTextActive,
            ]}
          >
        ...
        <TouchableOpacity
          style={[
            stylesLocal.tabButton,
            activeTab === 'options' && stylesLocal.tabButtonActive,
          ]}
        ...
        >
          <Text
            style={[
              stylesLocal.tabText,
              activeTab === 'options' && stylesLocal.tabTextActive,
            ]}
          >
          ...
          <View style={stylesLocal.modalContent}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (
                    ...
                    editingPresetIndex < updated.length
                  ) {
            ...
            <ScrollView style={uiStyles.scrollMaxHeight260}>
              {awayPresets.map((preset, idx) => (
                <View key={`${preset}-${idx}`} style={stylesLocal.presetItem}>
                  <View style={stylesLocal.presetRow}>
                    ...
                    <TouchableOpacity
                      style={[
                        ...
                        awaySelectedPreset === preset &&
                          stylesLocal.selectButtonActive,
                      ]}
                      onPress={async () => {
                        const next =
                          awaySelectedPreset === preset ? '' : preset;
                        ...
                        if (next) {
                          if (
                            !awayDefaultReason.trim() ||
                            awayDefaultReason === prevSelected
                          ) {
                            setAwayDefaultReason(next);
                            await settingsService.setSetting(
                              'awayDefaultReason',
                    ...
                    >
                      <Text
                        style={[
                          ...
                          awaySelectedPreset === preset &&
                            stylesLocal.selectButtonTextActive,
                        ]}
                      >
                        {awaySelectedPreset === preset
      ...
      >
        <View style={stylesLocal.modalContainer}>
          <View style={stylesLocal.modalContent}>
            <Text style={stylesLocal.modalTitle}>
              {t('Auto-answer presets', { _tags: tags })}
            </Text>
            <View style={stylesLocal.modalRow}>
              <TextInput
                style={stylesLocal.modalInput}
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  if (
                    editingAutoAnswerIndex !== null &&
                    editingAutoAnswerIndex >= 0 &&
                    editingAutoAnswerIndex < updated.length
                  ) {
                ...
                <View key={`${preset}-${idx}`} style={stylesLocal.presetItem}>
                  <View style={stylesLocal.presetRow}>
                    ...
                    <TouchableOpacity
                      style={stylesLocal.removeButton}
                      onPress={async () => {
                        const updated = awayAutoAnswerMessages.filter(
                          (_, i) => i !== idx,
                        );
                        setAwayAutoAnswerMessages(updated);

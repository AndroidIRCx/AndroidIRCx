...
export const QueryEncryptionMenu: React.FC<QueryEncryptionMenuProps> = ({
  ...
  const getNetworkForStorage = useCallback((): string => {
    ...
    console.log(
      '[QueryEncryptionMenu] getNetworkForStorage - network prop:',
      network,
    ...
    switch (action) {
      case 'enc_share':
        try {
          ...
          activeIrc.sendRaw(
            `PRIVMSG ${nick} :!enc-offer ${JSON.stringify(bundle)}`,
          );
            ...
            [
              {
                ...
                onPress: async () => {
                  if (!status.verified) {
                    ...
                    setActionMessage(
                      t('Key marked verified for {nick}', { nick }),
                    );
                  }
                },
              },

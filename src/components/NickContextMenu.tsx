...
interface NickContextMenuProps {
  ...
  initialUserHostInfo?: UserHostInfo | null;
...
export const NickContextMenu: React.FC<NickContextMenuProps> = ({
  ...
  const canMonitor = Boolean(
    activeIrc?.capEnabledSet &&
    activeIrc.capEnabledSet.has &&
    activeIrc.capEnabledSet.has('monitor'),
  );
  const isIgnored = nick
    ? userManagementService.isUserIgnored(nick, undefined, undefined, network)
    : false;
  ...
  const [userHostInfo, setUserHostInfo] = useState<{
    user: string;
    host: string;
  } | null>(null);
  ...
  const [showCallActionsInNickMenu, setShowCallActionsInNickMenu] =
    useState(false);
  ...
  useEffect(() => {
    ...
    setUserHostInfo(initialUserHostInfo || null);
  }, [
    initialUserHostInfo,
    initialUserHostInfo?.host,
    initialUserHostInfo?.user,
    nick,
    visible,
  ]);
  ...
  useEffect(() => {
    if (visible && network) {
      ...
      setServiceCommands(
        nickCommands.map(({ command, serviceNick }) => ({
          command,
          serviceNick,
        })),
      );
  ...
  useEffect(() => {
    if (visible) {
      const loadSettings = async () => {
        const confirm = await settingsService.getSetting(
          'confirmBeforeKickBan',
          NEW_FEATURE_DEFAULTS.confirmBeforeKickBan,
        );
        const banType = await settingsService.getSetting(
          'defaultBanType',
          NEW_FEATURE_DEFAULTS.defaultBanType,
        );
        const reasons = await settingsService.getSetting(
  ...
  const isCurrentUserHalfOp =
    ...
    false;
  const isCurrentUserOp =
    currentUser?.modes.some(mode => ['o', 'a', 'q'].includes(mode)) || false;
  const isHistoricalPresence =
    sourceMessageType === 'part' || sourceMessageType === 'quit';
  const canApplyChannelBan = isCurrentUserHalfOp || isCurrentUserOp;
  const canKickBan = canApplyChannelBan && !isHistoricalPresence;

  // Execute kick/ban directly without modal
  const executeKickBanDirect = (
    ...
    >
      ...
      >
        <View style={styles.contextBox}>
          <View style={styles.contextHeaderRow}>
            ...
            <TouchableOpacity
              style={styles.contextCopyButton}
          ...
          >
            ...
            >
              <View style={styles.contextItemWithIcon}>
                <Icon
                  ...
                  style={styles.contextIcon}
                />
                <Text style={styles.contextText}>{t('WHOWAS')}</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.contextItem}
                ...
                >
                  <View style={styles.contextItemWithIcon}>
                    <Icon
                      ...
                      style={styles.contextIcon}
                    />
                    <Text style={styles.contextText}>{t('Copy hostmask')}</Text>
                  </View>
                </TouchableOpacity>
              </>
            )}
            ...
            {canMonitor && (
              <TouchableOpacity
                style={styles.contextItem}
                onPress={() => onAction('monitor_toggle')}
              >
                <View style={styles.contextItemWithIcon}>
                  <Icon
                    name="eye"
                  ...
                  />
                  <Text style={styles.contextText}>
                    {isMonitoring ? t('Unmonitor Nick') : t('Monitor Nick')}
                  </Text>
                </View>
              </TouchableOpacity>
            )}
            ...
            <TouchableOpacity
              style={styles.contextItem}
              onPress={() => setShowUserListGroup(prev => !prev)}
            >
              <View style={styles.contextItemWithIcon}>
                <Icon
                  name="users"
            ...
            {showUserListGroup && (
              <View style={styles.contextSubGroup}>
                <TouchableOpacity
                  style={styles.contextItem}
                  onPress={() => onAction(ignoreActionId)}
                >
                  <View style={styles.contextItemWithIcon}>
                    <Icon
                    ...
                    />
                    <Text style={styles.contextText}>
                      {isIgnored ? t('Unignore User') : t('Ignore User')}
                    </Text>
                  </View>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.contextItem}
                  onPress={() => onAction('blacklist')}
                >
                  <View style={styles.contextItemWithIcon}>
                    <Icon
                      name="list-alt"
                ...
                >
                  <View style={styles.contextItemWithIcon}>
                    ...
                    />
                    <Text style={styles.contextText}>
                      {userManagementService.getUserNote(nick || '', network)
                        ? t('Edit Note')
                        : t('Add Note')}
                    </Text>
                  </View>
              ...
              <View style={styles.contextSubGroup}>
                ...
                >
                  <View style={styles.contextItemWithIcon}>
                    ...
                    />
                    <Text style={styles.contextText}>
                      {t('Request DM Key (36s)')}
                    </Text>
                  </View>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.contextItem}
                  onPress={() => onAction('enc_verify')}
                >
                  <View style={styles.contextItemWithIcon}>
                    <Icon
                      name="check-circle"
                ...
                {allowQrVerification && (
                  <>
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_qr_show_bundle')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="qrcode"
                        ...
                        />
                        <Text style={styles.contextText}>
                          {t('Share Key Bundle QR')}
                        </Text>
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_qr_show_fingerprint')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="fingerprint"
                        ...
                        />
                        <Text style={styles.contextText}>
                          {t('Show Fingerprint QR (Verify)')}
                        </Text>
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_qr_scan')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="camera"
                ...
                {allowFileExchange && (
                  <>
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_share_file')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="file-export"
                    ...
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_import_file')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="file-import"
                ...
                {allowNfcExchange && (
                  <>
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_share_nfc')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="wifi"
                    ...
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('enc_receive_nfc')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
                          name="wifi"
            ...
            {(isCurrentUserOp || isCurrentUserHalfOp) && (
              <>
                ...
                {showOpsGroup && (
                  <View style={styles.contextSubGroup}>
                    {isCurrentUserHalfOp &&
                      !isHistoricalPresence &&
                      targetUser && (
                        <>
                          {targetUser?.modes.includes('v') ? (
                            <TouchableOpacity
                              style={styles.contextItem}
                              onPress={() => onAction('take_voice')}
                            >
                              <View style={styles.contextItemWithIcon}>
                                <Icon
                    ...
                    {isCurrentUserOp && !isHistoricalPresence && targetUser && (
                      <>
                        {targetUser?.modes.includes('h') ? (
                          <TouchableOpacity
                            style={styles.contextItem}
                            onPress={() => onAction('take_halfop')}
                          >
                            <View style={styles.contextItemWithIcon}>
                              <Icon
                                name="user-minus"
                      ...
                      <>
                        {canKickBan && (
                          <>
                            <TouchableOpacity
                              style={styles.contextItem}
                              onPress={() => handleKickBanPress('kick', false)}
                            >
                              <View style={styles.contextItemWithIcon}>
                                <Icon
                                  name="sign-out-alt"
                            ...
                            <TouchableOpacity
                              style={styles.contextItem}
                              onPress={() => handleKickBanPress('kick', true)}
                            >
                              <View style={styles.contextItemWithIcon}>
                                <Icon
                                  name="sign-out-alt"
                        ...
                        <TouchableOpacity
                          style={styles.contextItem}
                          onPress={() => handleKickBanPress('ban', false)}
                        >
                          <View style={styles.contextItemWithIcon}>
                            <Icon
                              name="ban"
                        ...
                        {canKickBan && (
                          <>
                            <TouchableOpacity
                              style={styles.contextItem}
                              onPress={() =>
                                handleKickBanPress('kickban', false)
                              }
                            >
                              <View style={styles.contextItemWithIcon}>
                                <Icon
                            ...
                            <TouchableOpacity
                              style={styles.contextItem}
                              onPress={() =>
                                handleKickBanPress('kickban', true)
                              }
                            >
                              <View style={styles.contextItemWithIcon}>
                                <Icon
            ...
            {serviceCommands.length > 0 && (
              <>
                ...
                <TouchableOpacity
                  style={styles.contextItem}
                  onPress={() => setShowServiceGroup(prev => !prev)}
                >
                  <View style={styles.contextItemWithIcon}>
                    <Icon
                      name="server"
                  ...
                  <View style={styles.contextSubGroup}>
                    {serviceCommands.map(({ command, serviceNick }) => (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          activeIrc.sendRaw(
                            `PRIVMSG ${serviceNick} :${command.name} ${nick}`,
                          );
                          onClose();
                        }}
                      >
                    ...
                    <TouchableOpacity
                      style={styles.contextItem}
                      onPress={() => onAction('show_all_commands')}
                    >
                      <View style={styles.contextItemWithIcon}>
                        <Icon
          ...
          <View style={styles.contextFooter}>
            <TouchableOpacity
              style={styles.contextFooterButton}
              onPress={onClose}
            >
      ...
      <KickBanModal
        ...
        onConfirm={options => {
          ...
          if (
            options.ban &&
            options.unbanAfterSeconds &&
            options.unbanAfterSeconds > 0
          ) {
            setTimeout(() => {
      ...
      >
        <TouchableOpacity
          style={styles.contextOverlay}
          activeOpacity={1}
          onPress={() => setShowKillReasonModal(false)}
        >
          <View style={styles.contextBox}>
            <View style={styles.contextHeaderRow}>
              <View style={styles.contextHeaderText}>
                <Text style={styles.contextTitle}>
                  {t('KILL {nick}').replace('{nick}', nick || '')}
                </Text>

...
export const HelpTroubleshootingScreen: React.FC<
  HelpTroubleshootingScreenProps
> = ({ visible, onClose }) => {
  const t = useT();
    ...
    >
      <HelpSection title={t('Connection Issues')}>
        <HelpSubsection title={t("Can't Connect to Server")}>
          <HelpParagraph>
            {t('Symptoms: Connection timeout, "Unable to connect"')}
          </HelpParagraph>
          ...
          <HelpBullet>
            {t('Try different port (6667 for non-SSL, 6697 for SSL)')}
          </HelpBullet>
          ...
          <HelpBullet>
            {t('Try connecting via mobile data instead of WiFi')}
          </HelpBullet>
        </HelpSubsection>

        <HelpSubsection title={t('Disconnects Frequently')}>
          <HelpParagraph>
            {t('Symptoms: Random disconnections, "Connection lost"')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t(
        ...
        <HelpSubsection title={t('SSL/TLS Errors')}>
          <HelpParagraph>
            {t('Symptoms: "SSL handshake failed", "Certificate error"')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t('Verify server supports SSL on that port (usually 6697)')}
          </HelpBullet>
          <HelpBullet>
            {t('Try disabling "Verify SSL Certificate" in network settings')}
          </HelpBullet>
      ...
      <HelpSection title={t('Authentication Problems')}>
        <HelpSubsection title={t('Nickname Already in Use')}>
          <HelpParagraph>
            {t('Symptoms: "Nickname is already in use"')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>{t('Try alternate nickname')}</HelpBullet>
          <HelpBullet>
            {t('Wait a few minutes (old session timeout)')}
          </HelpBullet>
          <HelpBullet>
            {t('Use /msg NickServ GHOST YourNick password')}
          </HelpBullet>
          <HelpBullet>
            {t(
              'Change nickname: dropdown (▼) → Choose Network → Edit → Nickname',
            )}
          </HelpBullet>
        </HelpSubsection>
        ...
        <HelpSubsection title={t("NickServ Won't Identify")}>
          <HelpParagraph>
            {t('Symptoms: "Password incorrect", Not identified')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>{t('Verify password is correct')}</HelpBullet>
          <HelpBullet>
            {t('Wait for NickServ notice before identifying')}
          </HelpBullet>
          <HelpBullet>
            {t('Manually identify: /msg NickServ IDENTIFY password')}
          </HelpBullet>
        ...
        <HelpSubsection title={t('SASL Authentication Failed')}>
          <HelpParagraph>
            {t('Symptoms: "SASL authentication failed"')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
      ...
      <HelpSection title={t('Message Issues')}>
        <HelpSubsection title={t('Messages Not Sending')}>
          <HelpParagraph>
            {t("Symptoms: Message appears locally but others don't see it")}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          ...
          <HelpBullet>
            {t('Ensure you have voice (+v) if channel is moderated')}
          </HelpBullet>
        </HelpSubsection>

        <HelpSubsection title={t('Keyboard Covers Input')}>
          <HelpParagraph>
            {t('Symptoms: Keyboard overlaps the message input')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t('Go to Settings  Display & UI  Keyboard Avoiding')}
          </HelpBullet>
          <HelpBullet>
            {t('Android: try Behavior = Height (default) or Padding')}
          </HelpBullet>
          <HelpBullet>
            {t('iOS: try Behavior = Padding or Position')}
          </HelpBullet>
          <HelpBullet>
            {t('Adjust Keyboard Vertical Offset (e.g., 16-48)')}
          </HelpBullet>
          <HelpBullet>
            {t(
        ...
        <HelpSubsection title={t("Can't See Messages")}>
          <HelpParagraph>
            {t('Symptoms: Channel appears empty, no messages visible')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t('Check Settings → Display & UI → Show Raw Commands')}
          </HelpBullet>
          <HelpBullet>
            {t('Verify filters in Settings → Messages & History')}
          </HelpBullet>
        ...
        <HelpSubsection title={t('Encrypted Messages Show as Gibberish')}>
          <HelpParagraph>
            {t('Symptoms: Messages display as random characters')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
      ...
      <HelpSection title={t('Channel Problems')}>
        <HelpSubsection title={t("Can't Join Channel")}>
          <HelpParagraph>
            {t('Symptoms: "Cannot join channel", Error 473-475')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t('+i (Invite-only): Ask operator for invite')}
          </HelpBullet>
          <HelpBullet>
            {t(
        ...
        <HelpSubsection title={t("Can't Send Messages in Channel")}>
          <HelpParagraph>
            {t('Symptoms: Message rejected, channel is quiet')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
        ...
        <HelpSubsection title={t("Channel Won't Stay Open")}>
          <HelpParagraph>
            {t('Symptoms: Channel tab closes automatically')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
          <HelpBullet>
            {t(
              'Add it to auto-join: dropdown (▼) → Choose Network → Edit → Auto-Join Channels',
            )}
          </HelpBullet>
      ...
      <HelpSection title={t('App Performance')}>
        <HelpSubsection title={t('App is Slow/Laggy')}>
          ...
          <HelpBullet>
            {t('Reduce message batch size: Settings → Performance')}
          </HelpBullet>
      ...
      <HelpSection title={t('Notification Issues')}>
        <HelpSubsection title={t('Not Receiving Notifications')}>
          ...
          <HelpBullet>
            {t('Verify Settings → Notifications → Enabled')}
          </HelpBullet>
      ...
      <HelpSection title={t('Media/File Issues')}>
        <HelpSubsection title={t("Can't Send Media")}>
          <HelpParagraph>
            {t('Symptoms: Attachment button missing/disabled')}
          </HelpParagraph>
          <HelpParagraph>{t('Solutions:')}</HelpParagraph>
        ...
        <HelpSubsection title={t("Media Won't Download")}>
          ...
          <HelpBullet>
            {t('Enable auto-download: Settings → Media → Auto-Download Media')}
          </HelpBullet>
        ...
        <HelpSubsection title={t('Media Appears Encrypted')}>
          ...
          <HelpBullet>
            {t('Check Security → Manage Encryption Keys')}
          </HelpBullet>
        </HelpSubsection>
      </HelpSection>
      ...
      <HelpSection title={t('Account & Settings')}>
        <HelpSubsection title={t('Lost PIN/Password')}>
          ...
          <HelpBullet>
            {t('Last resort: Clear app data (loses all data)')}
          </HelpBullet>
        </HelpSubsection>
      ...
      <HelpSection title={t('Still Need Help?')}>
        <HelpSubsection title={t('Get Support')}>
          ...
          <HelpBullet>
            {t(
              'GitHub Issues: https://github.com/AndroidIRCx/AndroidIRCx/issues',
            )}
          </HelpBullet>
        </HelpSubsection>

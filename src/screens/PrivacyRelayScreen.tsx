...
export const PrivacyRelayScreen: React.FC<PrivacyRelayScreenProps> = ({
  ...
  const syncPurchaseWithBackend = useCallback(
    async (purchaseToken: string, basePlanId?: string | null) => {
      ...
      try {
        await privacyRelayService.registerPurchaseWithBackend(
          purchaseToken,
          basePlanId ?? null,
        );
        setBackendStatus(t('Fetching TURN credentials from backend...'));
        const credentials =
          await privacyRelayService.fetchTurnCredentials(purchaseToken);
        ...
        setBackendStatus(
          t('TURN credentials loaded. TTL: {ttl}s', {
            ttl: String(credentials.ttl || 0),
          }),
        );
        const alreadyAutoEnabled =
    ...
    try {
      ...
      Alert.alert(
        ...
        error instanceof Error
          ? error.message
          : t('Failed to load Privacy Relay subscription.'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handlePurchase = useCallback(
    async (planId: string) => {
      const selectedOffer =
        offers.find(offer => offer.basePlanId === planId) || null;
      ...
      try {
        ...
        if (Platform.OS === 'ios') {
          ...
          if (!selectedOffer?.offerToken) {
            throw new Error(
              'Missing Google Play offer token for this base plan.',
            );
          }
          request = {
            request: {
  ...
  const handleRestore = useCallback(async () => {
    ...
    try {
      ...
      const restoredPurchase = relayPurchases.find(purchase =>
        Boolean(getPurchaseReceipt(purchase)),
      );
  ...
  const handleTestCredentials = useCallback(async () => {
    ...
    if (!purchaseToken) {
      Alert.alert(
        t('No subscription'),
        t('Buy or restore Privacy Relay first.'),
      );
      return;
    }
    ...
    try {
      ...
      await syncPurchaseWithBackend(
        purchaseToken,
        privacyRelayService.getSubscription()?.basePlanId ?? null,
      );
      Alert.alert(
        t('Relay ready'),
        t('TURN credentials were fetched successfully.'),
      );
    } catch (error) {
      Alert.alert(
        t('Relay test failed'),
        error instanceof Error
          ? error.message
          : t('Unable to fetch TURN credentials.'),
      );
    }
  ...
  const renderPlanCard = (planId: string, fallbackTitle: string) => {
    ...
    const isCurrent =
      subscription?.basePlanId === planId && relayState.hasSubscription;
      ...
      <View key={planId} style={styles.planCard}>
        <View style={styles.planHeader}>
          ...
          <Text style={styles.planPrice}>
            {offer?.price || product?.displayPrice || t('Unavailable')}
          </Text>
        </View>
        ...
        <TouchableOpacity
          ...
          disabled={
            isCurrent || isBusy || (!offer && Platform.OS === 'android')
          }
        >
          {isBusy ? (
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <View style={styles.statusCard}>
            ...
            <Text style={styles.statusText}>
              {relayState.hasSubscription
                ? t(
                    'Relay routing can be offered for calls that need TURN fallback.',
                  )
                : t(
                    'Direct P2P remains default. TURN relay is only offered to subscribed users.',
                  )}
            </Text>
            {!!backendStatus && (
          ...
          <View style={styles.infoCard}>
            ...
            >{`Host: ${relayState.relayServer.host}`}</Text>
            <Text
              style={styles.infoLine}
            >{`STUN: ${relayState.relayServer.stunUrls[0]}`}</Text>
            <Text
              style={styles.infoLine}
            >{`TURN TLS: turns:${relayState.relayServer.host}:${relayState.relayServer.tlsPort}?transport=tcp`}</Text>
            <Text style={styles.infoFootnote}>
              {t(
                'Credentials are not embedded in the app. They must be issued by your backend per session.',
              )}
            </Text>
          </View>
          ...
          {loading ? (
            ...
            <>
              {renderPlanCard(
                PRIVACY_RELAY_BASE_PLAN_IDS.monthly,
                t('Monthly'),
              )}
          ...
          <TouchableOpacity
            style={[
              ...
              (syncingBackend || !subscription) && styles.disabledButton,
            ]}
          ...
          {!!lastCredentialFetch && (
            <View style={styles.infoCard}>
              ...
              <Text
                style={styles.infoLine}
              >{`Username: ${lastCredentialFetch.username}`}</Text>
              <Text
                style={styles.infoLine}
              >{`TTL: ${lastCredentialFetch.ttl}s`}</Text>
              <Text
                style={styles.infoLine}
              >{`Call ID: ${lastCredentialFetch.callId}`}</Text>
            </View>
          )}
        </ScrollView>
      </ModalSafeArea>

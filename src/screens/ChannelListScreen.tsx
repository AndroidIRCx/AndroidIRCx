...
import {
  ...
  ChannelListItem,
  ChannelListFilter,
} from '../services/ChannelListService';
...
export const ChannelListScreen: React.FC<ChannelListScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const service =
      connection?.channelListService || singletonChannelListService;
  ...
  const handleToggleFavorite = useCallback(
    async (channel: ChannelListItem) => {
      ...
      const isFavorite = channelFavoritesService.isFavorite(
        baseNetworkId,
        channel.name,
      );
      if (isFavorite) {
  ...
  const renderChannelItem = useCallback(
    ({ item }: { item: ChannelListItem }) => {
      const isFavorite = network
        ? channelFavoritesService.isFavorite(network, item.name)
        : false;
  ...
  const getItemLayout = useCallback(
    (_data: ArrayLike<ChannelListItem> | null | undefined, index: number) => ({
    ...
    >
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            onPress={loadChannelList}
            style={styles.refreshButton}
          >
            <Text style={[styles.refreshText, { color: colors.onPrimary }]}>
              {t('Refresh')}
            </Text>
          </TouchableOpacity>
        </View>
          ...
          >
            <TextInput
              ...
              onChangeText={text =>
                setFilter({
                  ...filter,
                  minUsers: text ? parseInt(text, 10) : undefined,
                })
              }
            />
            <TextInput
              ...
              onChangeText={text =>
                setFilter({
                  ...filter,
                  maxUsers: text ? parseInt(text, 10) : undefined,
                })
              }
            />
            <TextInput
              ...
              onChangeText={text =>
                setFilter({ ...filter, namePattern: text || undefined })
              }
            />
          </View>
        )}
        ...
        >
          <TouchableOpacity
            style={[
              styles.sortButton,
              sortBy === 'users' && styles.sortButtonActive,
            ]}
            onPress={() => {
              if (sortBy === 'users') {
          ...
          >
            <Text
              style={[
                ...
                { color: sortBy === 'users' ? colors.primary : colors.text },
              ]}
            >

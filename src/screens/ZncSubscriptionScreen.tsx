...
export const ZncSubscriptionScreen: React.FC<ZncSubscriptionScreenProps> = ({
  ...
  const [selectedAccount, setSelectedAccount] = useState<ZncAccount | null>(
    null,
  );
  ...
  const handlePurchaseError = useCallback(
    (error: PurchaseError) => {
      ...
      if (error.code !== ErrorCode.UserCancelled) {
        Alert.alert(
          t('Purchase Failed'),
          error.message || t('Unable to complete purchase.'),
        );
      }
    },
    [t],
    ...
    async (purchaseToken: string, username: string) => {
      ...
      try {
        ...
        Alert.alert(
          t('Success'),
          t(
            'Your ZNC account has been created successfully! Username: {username}',
        ...
        if (
          errorMessage.includes('znc_username_exists') ||
          ...
          Alert.alert(
            ...
            error.message || t('Unable to register ZNC account.'),
          );
        }
      } finally {
    ...
    async (purchase: Purchase) => {
      ...
      try {
        ...
        if (token && pendingUsernameRef.current) {
          ...
          console.error(
            'No purchase token or username found:',
            { token: !!token, username: !!pendingUsernameRef.current },
            purchase,
          );
        ...
        Alert.alert(
          t('Error'),
          t('Failed to complete purchase. Please try again.'),
        );
      } finally {
  ...
  const initializeIap = useCallback(async () => {
    try {
      ...
      const products =
        (await RNIap.fetchProducts({ skus: [ZNC_PRODUCT_ID], type: 'subs' })) ??
        [];
      ...
      if (products && products.length > 0) {
        ...
        try {
          ...
          console.log(
            'Available purchases (for debugging):',
      ...
      const sub = products.find(
        (item): item is ProductSubscription =>
          item.id === ZNC_PRODUCT_ID && item.type === 'subs',
      );
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      Alert.alert(
        t('Error'),
        error.message ||
          t('Failed to refresh accounts. Please check your connection.'),
      );
    } finally {
      if (isMountedRef.current) {
        setRefreshing(false);
  ...
  const handlePurchaseNew = async () => {
    if (!iapConnected) {
      Alert.alert(
        t('Error'),
        t('Store not available. Please check your connection and try again.'),
      );
      return;
    }
    ...
    if (!subscription) {
      Alert.alert(
        t('Error'),
        t(
          'Subscription product is not available in the store. This may be due to store configuration or testing environment.',
        ),
      );
      return;
    }
    ...
    if (Platform.OS === 'android') {
      ...
      if (!resolvedToken) {
        Alert.alert(
          t('Store Error'),
          t(
            'Subscription offer is not available right now. Please try again in a few moments.',
          ),
        );
        return;
      }
  ...
  const handleConfirmPurchase = async () => {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(username)) {
      Alert.alert(
        t('Error'),
        t('Username can only contain letters, numbers, underscore and dash.'),
      );
      return;
    }
    ...
    if (username.length < 3 || username.length > 20) {
      Alert.alert(
        t('Error'),
        t('Username must be between 3 and 20 characters.'),
      );
      return;
    }
    ...
    if (existing) {
      Alert.alert(
        t('Error'),
        t('You already have an account with this username.'),
      );
      return;
    }
    ...
    try {
      const isAvailable =
        await subscriptionService.checkUsernameAvailability(username);
      if (!isAvailable) {
        setPurchasing(false);
        Alert.alert(
          t('Username Taken'),
      ...
      console.warn(
        'Username availability check failed, continuing with purchase:',
        error,
      );
    }
    ...
    try {
      ...
      if (error.code !== ErrorCode.UserCancelled) {
        Alert.alert(
          t('Error'),
          error.message || t('Failed to start purchase.'),
        );
      }
    }
  };
  ...
  const handleRestorePurchases = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          'ZNC Restore: Restored purchases:',
          restoredPurchases.length,
        );
      } catch (error) {
      ...
      console.log(
        'ZNC Restore: Product IDs:',
        allPurchases.map(p => p.productId),
      );

      const zncPurchases = allPurchases.filter(
        p => p.productId === ZNC_PRODUCT_ID,
      );
      ...
      if (zncPurchases.length === 0) {
        ...
        console.log(
          'ZNC Restore: No ZNC purchases. Available products:',
          JSON.stringify(
            allPurchases.map(p => ({
        ...
        Alert.alert(
          t('No Purchases'),
          t('No ZNC subscriptions found to restore.'),
        );
        return;
      }
      ...
      for (const purchase of zncPurchases) {
        ...
        if (token) {
          tokens.push(token);
          console.log(
            'ZNC Restore: Extracted token (first 50 chars):',
            token.substring(0, 50),
          );
        } else {
          console.log(
            'ZNC Restore: No token found for purchase:',
      ...
      if (tokens.length === 0) {
        Alert.alert(
          t('No Purchases'),
          t('No valid subscriptions found to restore.'),
        );
        return;
      }

      console.log(
        'ZNC Restore: Calling restorePurchases with',
        tokens.length,
        'tokens',
      );
      ...
      if (result.restored > 0) {
        Alert.alert(
          t('Restored'),
          t('{count} subscription(s) restored successfully.', {
      ...
      Alert.alert(
        t('Error'),
        error.message || t('Failed to restore purchases.'),
      );
    } finally {
      setRestoring(false);
    }
  ...
  const getUnlockedPassword = useCallback(
    async (account: ZncAccount): Promise<string | null> => {
  ...
  const handleAddToNetwork = async (account: ZncAccount) => {
    if (!account.zncUsername || !isZncAccountReady(account)) {
      Alert.alert(
        t('Error'),
        t('Account credentials not ready. Please refresh status first.'),
      );
      return;
    }
  ...
  const handleNetworkSelected = async (network: IRCNetworkConfig) => {
    ...
    try {
      ...
      const password = await subscriptionService.getAccountPassword(
        selectedAccount.id,
      );
      if (!password) {
        Alert.alert(t('Error'), t('Credentials not available.'));
      ...
      const serverConfig = subscriptionService.generateServerConfig(
        selectedAccount,
        password,
      );
      ...
      const existingServer = network.servers?.find(s => {
        ...
        return (
          current.id === serverConfig.id ||
          (current.connectionType === 'znc' &&
            current.username === serverConfig.username)
        );
      });
      ...
      if (existingServer) {
        Alert.alert(
          t('Server Exists'),
          t(
            'This ZNC account is already configured in {network}. Do you want to update it?',
            {
          ...
          [
            { text: t('Cancel'), style: 'cancel' },
            {
              text: t('Update'),
              onPress: () =>
                updateServerInNetwork(network, serverConfig, existingServer.id),
            },
          ],
        );
        return;
      ...
      Alert.alert(
        t('Error'),
        error.message || t('Failed to add ZNC to network.'),
      );
    }
  };

  const addServerToNetwork = async (
    ...
    try {
      ...
      Alert.alert(
        t('Success'),
        t(
          'ZNC server added to {network}. You can now connect using the ZNC server.',
          {
            network: network.name,
          },
        ),
      ...
      Alert.alert(
        t('Error'),
        error.message || t('Failed to save network configuration.'),
      );
    }
  };
  ...
  const updateServerInNetwork = async (
    ...
    try {
      ...
      Alert.alert(
        t('Error'),
        error.message || t('Failed to update network configuration.'),
      );
    }
  };
  ...
  const handleRefreshAccount = async (account: ZncAccount) => {
    try {
      const updatedAccount = await subscriptionService.refreshAccountStatus(
        account.id,
      );
      if (updatedAccount) {
        ...
        Alert.alert(
          t('Error'),
          t('Could not refresh account status. Account may not exist.'),
        );
      }
    } catch (error: any) {
      ...
      Alert.alert(
        t('Error'),
        error.message ||
          t('Failed to refresh account status. Please check your connection.'),
      );
    }
  };
  ...
  const handleRemoveFromNetwork = async (account: ZncAccount) => {
    ...
    Alert.alert(
      ...
      t(
        'Remove ZNC server from {network}? This will not cancel your subscription.',
        {
        ...
        { text: t('Cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const network = await settingsService.getNetwork(
              ...
              Alert.alert(
                t('Error'),
                error.message || t('Failed to remove from network.'),
              );
            }
          },
        },
  ...
  const handleDeleteAccount = (account: ZncAccount) => {
    Alert.alert(
      t('Delete Account'),
      t(
        'Delete this ZNC account from your local storage? This will NOT cancel your subscription - you can restore it later using "Restore Purchases".',
      ),
      [
        { text: t('Cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                t('Deleted'),
                t('Account removed from local storage.'),
              );
            } catch (error: any) {
              Alert.alert(
                t('Error'),
  ...
  const renderEmpty = () => {
    ...
    const visibleAccounts = accounts.filter(
      account => !(account.status === 'pending' && !account.zncUsername),
    );
    ...
    <>
          ...
          {loading ? (
            ...
            <FlatList
              data={accounts.filter(
                account =>
                  !(account.status === 'pending' && !account.zncUsername),
              )}
              ...
              {purchasing || refreshingOffers ? (
                <>
                  ...
                  <Text style={styles.purchaseButtonText}>
                    {purchasing
                      ? t('Creating ZNC Account, Please Wait...')
                      : t('Loading Offers...')}
                  </Text>
                </>
              ) : (
      ...
      >
        <View style={styles.modalOverlay}>
          <View
            style={[styles.usernameModal, { backgroundColor: colors.surface }]}
          >
            ...
            >
              {t(
                'Enter a username for your ZNC account. This will be used to log in to ZNC.',
              )}
            </Text>

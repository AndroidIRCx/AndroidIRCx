...
export const useSettingsNotifications = (): UseSettingsNotificationsReturn => {
  const t = useT();
  const tags =
    'screen:settings,file:useSettingsNotifications.ts,feature:settings';

  const [notificationPrefs, setNotificationPrefs] =
    useState<NotificationPreferences>(notificationService.getPreferences());
    ...
    if (Platform.OS === 'android') {
      try {
        ...
        Alert.alert(
          t('Error', { _tags: tags }),
          t(
            'Failed to open battery optimization settings. Please enable it manually in system settings.',
            { _tags: tags },
          ),
        );
      }

...
export const useTabContextMenu = (params: UseTabContextMenuParams) => {
  ...
  const handleTabLongPress = useCallback(
    (tab: ChannelTab) => {
      ...
      const updateTabOptionsModal = (title: string, options: TabOption[]) => {
      ...
      updateTabOptionsModal(
        tab.type === 'server'
          ? `Server: ${tab.networkId}`
          : `${tab.type === 'channel' ? t('Channel') : t('Query')}: ${tab.name}`,
        [
          { text: t('Loading...'), icon: 'progress-clock', onPress: () => {} },
          {
            text: t('Cancel'),
            style: 'cancel',
            onPress: () => useUIStore.getState().setShowTabOptionsModal(false),
          },
        ],
      );
      ...
      (async () => {
        ...
        if (tab.type === 'server') {
          ...
          const isTabConnected =
            !!tabConnection?.ircService.getConnectionStatus();
          ...
          const isServerOper =
            typeof svc?.isServerOper === 'function'
              ? svc.isServerOper()
              : false;
          ...
          if (isTabConnected) {
            ...
            if (networkConfig?.clientCert) {
              options.push({
                ...
                onPress: async () => {
                  try {
                    const fingerprint =
                      certificateManager.extractFingerprintFromPem(
                        networkConfig.clientCert!,
                      );
                    if (!fingerprint) {
                      ...
                      [
                        {
                          ...
                          onPress: () => {
                            ...
                            safeAlert(
                              t('Copied'),
                              t('Fingerprint copied to clipboard'),
                            );
                          },
                        },
                        {
                    ...
                    safeAlert(
                      t('Error'),
                      t('Failed to extract certificate fingerprint'),
                    );
                  }
                },
              });
              ...
              options.push({
                ...
                onPress: async () => {
                  try {
                    ...
                    const nickServConnection = connectionManager.getConnection(
                      tab.networkId,
                    );
                    if (nickServConnection?.ircService) {
                      nickServConnection.ircService.sendRaw(
                        `PRIVMSG NickServ :CERT ADD ${formatted}`,
                      );
                      safeAlert(
                        t('Sent'),
                        t('Certificate fingerprint sent to NickServ'),
                      );
                    } else {
                    ...
                    safeAlert(
                      t('Error'),
                      t('Failed to send certificate fingerprint'),
                    );
                  }
                },
              });
            ...
            options.push({
              ...
              icon: 'lan-connect',
              onPress: async () => {
                const networkConfig = await getNetworkConfigForId(
                  tab.networkId,
                );
                if (networkConfig) {
            ...
            options.push({
              ...
              onPress: () => {
                const ircopOptions: TabOption[] = [
                  ...
                  {
                    text: 'VERSION',
                    onPress: () => svc.sendCommand('VERSION'),
                  },
                  ...
                  { text: 'TRACE', onPress: () => svc.sendCommand('TRACE') },
                  {
                    text: 'REHASH',
                    onPress: () =>
                      safeAlert('REHASH', 'Rehash server configuration?', [
                        { text: 'Cancel', style: 'cancel' },
                        {
                          text: 'Rehash',
                          onPress: () => svc.sendCommand('REHASH'),
                        },
                      ]),
                  },
                  {
                    text: 'RESTART',
                    onPress: () =>
                      safeAlert('RESTART', 'Restart IRC server?', [
                        { text: 'Cancel', style: 'cancel' },
                        {
                          text: 'Restart',
                          onPress: () => svc.sendCommand('RESTART'),
                        },
                      ]),
                  },
                  {
                    text: 'DIE',
                    onPress: () =>
                      safeAlert('DIE', 'Shut down IRC server?', [
                        { text: 'Cancel', style: 'cancel' },
                        {
                          text: 'Shutdown',
                          onPress: () => svc.sendCommand('DIE'),
                        },
                      ]),
                  },
                  {
          ...
          const networkConfig = (await settingsService.loadNetworks()).find(
            n => n.name === tab.networkId || n.id === tab.networkId,
          );
          const hasFavoriteServer =
            networkConfig?.servers?.some(s => s.favorite) ||
            !!networkConfig?.defaultServerId;
          if (!hasFavoriteServer) {
            options.push({
              ...
              onPress: async () => {
                ...
                setTabs(prev => {
                  const updated = sortTabsGrouped(
                    prev.filter(tabItem => tabItem.networkId !== tab.networkId),
                    tabSortAlphabetical,
                  );
                  if (!updated.some(tabItem => tabItem.id === activeTabId)) {
                    const primaryServerId = primaryNetworkId
                      ? serverTabId(primaryNetworkId)
                      : '';
                    const fallbackId =
                      primaryServerId &&
                      updated.some(tabItem => tabItem.id === primaryServerId)
                        ? primaryServerId
                        : updated.find(tabItem => tabItem.type === 'server')
                            ?.id ||
                          updated[0]?.id ||
                          '';
                    if (fallbackId) {
                      setActiveTabId(fallbackId);
                      const fallbackTab = updated.find(
                        tabItem => tabItem.id === fallbackId,
                      );
                      if (fallbackTab) {
        ...
        if (tab.type === 'channel') {
          try {
            isBookmarked = await channelNotesService.isBookmarked(
              tab.networkId,
              tab.name,
            );
          } catch {
          ...
          const channelEncLabel = tab.sendEncrypted
            ? t('Send Plaintext (Unlock)')
            : t('Send Encrypted (Lock)');
          options.push({
            ...
            onPress: async () => {
              ...
              setTabs(prev =>
                prev.map(t =>
                  t.id === tab.id ? { ...t, sendEncrypted: nextValue } : t,
                ),
              );
              const svc =
                connectionManager.getConnection(tab.networkId)?.ircService ||
                ircService;
              svc.addMessage({
                type: 'notice',
                channel: tab.name,
                text: nextValue
                  ? `*** Channel encryption enabled for ${tab.name}`
                  : `*** Channel encryption disabled for ${tab.name}`,
                timestamp: Date.now(),
              });
              useUIStore.getState().setShowTabOptionsModal(false);
            },
          ...
          const hasQueryBundle = await encryptedDMService.isEncryptedForNetwork(
            queryNetworkId,
            tab.name,
          );

          options.push({
            text: t('Always Encrypt: {status}', {
              status: alwaysEncryptEnabled ? 'ON' : 'OFF',
            }),
            icon: 'shield-lock',
            onPress: async () => {
              const newValue =
                await channelEncryptionSettingsService.toggleAlwaysEncrypt(
                  tab.name,
                  tab.networkId,
                );
          ...
          options.push({
            ...
            onPress: async () => {
              ...
              const closePrivateMessage = await settingsService.getSetting(
                'closePrivateMessage',
                false,
              );
              if (closePrivateMessage) {
                const ircServices = await settingsService.getSetting(
                  'ircServices',
                  [
                    'nickserv',
                    'chanserv',
            ...
            onPress: async () => {
              try {
                ...
                const svc =
                  ...
                  ircService;
                svc.sendRaw(
                  `PRIVMSG ${tab.name} :!enc-offer ${JSON.stringify(bundle)}`,
                );
                svc.addMessage({
                  type: 'system',
                  channel: tab.name,
          ...
          const queryEncLabel = tab.sendEncrypted
            ? t('Send Plaintext (Unlock)')
            : t('Send Encrypted (Lock)');
          options.push({
            text: queryEncLabel,
            ...
            onPress: async () => {
              ...
              setTabs(prev =>
                prev.map(tabItem =>
                  tabItem.id === tab.id
                    ? { ...tabItem, sendEncrypted: nextValue }
                    : tabItem,
                ),
              );
              const svc =
              ...
              svc.addMessage({
                ...
                channel: tab.name,
                text: nextValue
                  ? `*** DM encryption enabled with ${tab.name}`
                  : `*** DM encryption disabled with ${tab.name}`,
                timestamp: Date.now(),
              });
              useUIStore.getState().setShowTabOptionsModal(false);
            },
          ...
          options.push({
            ...
            onPress: () => {
              ...
              (conn?.ircService || getActiveIRCService()).sendCommand(
                `WHOWAS ${tab.name}`,
              );
              useUIStore.getState().setShowTabOptionsModal(false);
            },
          ...
          options.push({
            ...
            onPress: async () => {
              const svc =
                connectionManager.getConnection(tab.networkId)
                  ?.userManagementService || getActiveUserManagementService();
        ...
        if (tab.type === 'channel') {
          options.push({
            ...
            onPress: async () => {
              try {
                await channelEncryptionService.generateChannelKey(
                  tab.name,
                  tab.networkId,
                );
                const svc =
          ...
          const hasEncKey = await channelEncryptionService.hasChannelKey(
            tab.name,
            tab.networkId,
          );
          ...
          options.push({
            text: isBookmarked
              ? t('Remove Channel Bookmark')
              : t('Bookmark Channel'),
            icon: isBookmarked ? 'bookmark-off' : 'bookmark',
            onPress: async () => {
              await channelNotesService.setBookmarked(
                tab.networkId,
                tab.name,
                !isBookmarked,
              );
          ...
          options.push({
            ...
            onPress: async () => {
              ...
              useUIStore.getState().setChannelNoteTarget({
                networkId: tab.networkId,
                channel: tab.name,
              });
          ...
          options.push({
            ...
            icon: 'history',
            onPress: async () => {
              const log = await channelNotesService.getLog(
                tab.networkId,
                tab.name,
              );
              useUIStore.getState().setChannelNoteTarget({
              ...
              useUIStore
                ...
                .setChannelLogEntries(
                  log.slice().sort((a, b) => a.timestamp - b.timestamp),
                );
          ...
          const channelServiceCommands =
            serviceCommandProvider.getServiceCommands(
              tab.networkId,
              'chanserv',
            );
          if (channelServiceCommands.length > 0) {
            options.push({
              ...
              onPress: () => {
                const chanserv =
                  channelServiceCommands[0]?.service || 'ChanServ';
                              ...
                              if (nick?.trim()) {
                                const svc =
                                  connectionManager.getConnection(tab.networkId)
                                    ?.ircService || ircService;
                                svc.sendRaw(
                                  `PRIVMSG ${chanserv} :KICK ${tab.name} ${nick.trim()}`,
                                );
                              }
                            },
                          },
                              ...
                              if (mask?.trim()) {
                                const svc =
                                  connectionManager.getConnection(tab.networkId)
                                    ?.ircService || ircService;
                                svc.sendRaw(
                                  `PRIVMSG ${chanserv} :BAN ${tab.name} ${mask.trim()}`,
                                );
                              }
                            },
                          },
                              ...
                              if (mask?.trim()) {
                                const svc =
                                  connectionManager.getConnection(tab.networkId)
                                    ?.ircService || ircService;
                                svc.sendRaw(
                                  `PRIVMSG ${chanserv} :UNBAN ${tab.name} ${mask.trim()}`,
                                );
                              }
                            },
                          },
                              ...
                              if (topic !== undefined) {
                                const svc =
                                  connectionManager.getConnection(tab.networkId)
                                    ?.ircService || ircService;
                                svc.sendRaw(
                                  `PRIVMSG ${chanserv} :TOPIC ${tab.name} ${topic}`,
                                );
                              }
                            },
                          },
                    ...
                    onPress: () => {
                      const svc =
                        connectionManager.getConnection(tab.networkId)
                          ?.ircService || ircService;
                      svc.sendRaw(
                        `PRIVMSG ${chanserv} :AKICK ${tab.name} LIST`,
                      );
                      useUIStore.getState().setShowTabOptionsModal(false);
                    },
                  },
                ...
                updateTabOptionsModal(
                  t('IRC Services: {channel}', { channel: tab.name }),
                  serviceOptions,
                );
              },
            });
        ...
        const isFav = await channelFavoritesService.isFavorite(
          baseNetworkId,
          tab.name,
        );
        if (isFav) {
          options.push({
            ...
            icon: 'star-off',
            onPress: async () => {
              await channelFavoritesService.removeFavorite(
                baseNetworkId,
                tab.name,
              );
              useUIStore.getState().setShowTabOptionsModal(true);
          ...
          // Only channels can be favorited
          options.push({
        ...
        debugLogger.warn(
          'tabContextMenu',
          'Failed to build tab context menu',
          error,
        );
        updateTabOptionsModal(t('Options'), [
          {

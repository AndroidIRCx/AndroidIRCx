  ...
  const handleConnect = useCallback(
    async (
      ...
      if (networkToUse?.identityProfileId) {
        ...
        identityProfile = profiles.find(
          p => p.id === networkToUse!.identityProfileId,
        );
      }
      if (identityProfile && networkToUse) {
        networkToUse = {
          ...
          altNick:
            identityProfile.altNick || networkToUse.altNick || 'AndroidIRCX_',
          realname:
            identityProfile.realname ||
            networkToUse.realname ||
            'AndroidIRCX User',
          ident: identityProfile.ident || networkToUse.ident || 'androidircx',
          sasl: identityProfile.saslAccount
            ? {
                account: identityProfile.saslAccount,
                password: identityProfile.saslPassword || '',
              }
            : networkToUse.sasl,
          nickservPassword:
            identityProfile.nickservPassword || networkToUse.nickservPassword,
          operUser: identityProfile.operUser || networkToUse.operUser,
          operPassword:
            identityProfile.operPassword || networkToUse.operPassword,
        };
      }
      if (networkToUse && overrides?.identity) {
        ...
        if (identityOverride.altNick)
          networkToUse.altNick = identityOverride.altNick;
      ...
      if (!networkToUse) {
        safeAlert(
          ...
          t('No network available. Please configure a network first.', {
      ...
      if (!serverToUse && networkToUse.defaultServerId) {
        serverToUse = networkToUse.servers.find(
          s => s.id === networkToUse.defaultServerId,
        );
      }
      if (!serverToUse) {
      ...
      if (networkToUse?.name && serverToUse?.hostname) {
        appendServerMessage(
          networkToUse.name,
          `Connecting to ${serverToUse.hostname}:${serverToUse.port || ''}...`,
        );
      }
      ...
      const existingServerTab = tabsRef.current.find(
        tabItem =>
          tabItem.type === 'server' &&
          (tabItem.networkId === desiredId ||
            tabItem.networkId === networkToUse?.name),
      );
      ...
      try {
        const safeConfig = {
          ...
          sasl: config.sasl
            ? {
                ...config.sasl,
                password: config.sasl.password ? '[redacted]' : '',
              }
            : undefined,
          proxy: config.proxy
            ? {
        ...
        console.log(
          'App: Attempting to connect to IRC server using ConnectionManager...',
          safeConfig,
        );
        ...
        const withServerTab = normalizedTabs.some(
          tabItem => tabItem.type === 'server',
        )
          ? normalizedTabs
          : [makeServerTab(finalId), ...normalizedTabs];
    ...
    async (serverArgs: any, activeIRCService: any) => {
      try {
        ...
        if (serverArgs.serverIndex) {
          ...
          if (
            currentNetwork &&
            currentNetwork.servers[serverArgs.serverIndex - 1]
          ) {
          ...
          serverConfig = network.servers.find(
            s => s.hostname.toLowerCase() === serverArgs.address.toLowerCase(),
          );
          ...
          if (!serverConfig) {
            serverConfig = {
              ...
              ssl:
                serverArgs.switches?.ssl ||
                serverArgs.switches?.starttls ||
                false,
            ...
            if (serverArgs.password)
              serverConfig.password = serverArgs.password;
          }

          // Apply identity if provided
          if (serverArgs.identity?.nick)
            network.nick = serverArgs.identity.nick;
          if (serverArgs.identity?.altNick)
            network.altNick = serverArgs.identity.altNick;
          if (serverArgs.identity?.name)
            network.realname = serverArgs.identity.name;
          if (serverArgs.identity?.email)
            network.realname = `${network.realname} <${serverArgs.identity.email}>`;
          ...
          if (serverArgs.joinChannels && serverArgs.joinChannels.length > 0) {
            network.autoJoinChannels = serverArgs.joinChannels.map(
              (j: any) => j.channel,
            );
          }
        }
        ...
        if (network && serverConfig) {
          ...
          if (serverArgs.switches?.newWindowNoConnect) {
            ...
            activeIRCService.addMessage({
              type: 'notice',
              text: t(
                '*** Network "{name}" created (not connected). Use /server -m {address} to connect.',
                {
                  name: network.name,
            ...
            const unsubscribeConnectionCreated =
              connectionManager.onConnectionCreated((networkId: string) => {
            ...
            if (serverArgs.joinChannels && serverArgs.joinChannels.length > 0) {
              const connection =
                (newConnectionId
                  ? connectionManager.getConnection(newConnectionId)
                  : undefined) || connectionManager.getActiveConnection();
        ...
        activeIRCService.addMessage({
          type: 'error',
          text: t('Failed to connect: {error}', {

  ...
  const processDeepLinkAfterWarning = useCallback(
    async (parsed: ReturnType<typeof parseIRCUrl>) => {
      ...
      try {
        ...
        let existingConnection: { networkId: string; ircService: any } | null =
          null;
        ...
        for (const context of allConnections) {
          const normalizedNetworkId = context.networkId
            .toLowerCase()
            .replace(/\s*\(\d+\)$/, ''); // Remove (2), (3) suffixes
        ...
        if (matchedNetwork) {
          ...
          logger.info(
            'deeplink',
            `No matching network, creating temporary config for ${parsed.server}`,
          );
        ...
        const serverToUse =
          networkToUse.servers.find(
            s =>
              s.hostname.toLowerCase() === parsed.server.toLowerCase() &&
              s.port === parsed.port,
        ...
        for (const context of allConnectionsNow) {
          ...
          if (network && network.servers) {
            for (const srv of network.servers) {
              if (
                srv.hostname.toLowerCase() === parsed.server.toLowerCase() &&
                srv.port === parsed.port
              ) {
                logger.info(
                  'deeplink',
        ...
        logger.error(
          'deeplink',
          `Error processing deep link: ${error?.message || String(error)}`,
        );
        safeAlert(
          t('Error'),
    ...
    async (url: string) => {
      ...
      if (!parsed.isValid) {
        safeAlert(
          t('Invalid IRC URL'),
          t(
            'The IRC URL format is invalid: {error}\n\nExpected: irc://server/channel',
            {
      ...
      if (parsed.password) {
        safeAlert(
          t('Security Warning'),
          t(
            'This URL contains a password which is visible in plain text. This is insecure.\n\nServer: {server}\n\nContinue anyway?',
            {
              server: displayName,
            },
          ),
  ...
  const handleUrl = useCallback(
    async (url: string | null) => {
      ...
      if (
        ...
        logger.info(
          'deeplink',
          `Ignoring duplicate URL received within 2 seconds: ${url}`,
        );
        return;
      }
      ...
      if (isAppLocked || !isFirstRunComplete) {
        logger.info(
          'deeplink',
          'App locked or first run not complete, queuing URL',
        );
  ...
  useEffect(() => {
    ...
    Linking.getInitialURL()
      ...
      .catch(error => {
        logger.error(
          'deeplink',
          `Error getting initial URL: ${error?.message || String(error)}`,
        );
      });

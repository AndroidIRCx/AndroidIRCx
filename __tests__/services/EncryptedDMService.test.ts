...
const mockGetSecret = jest.fn(async (k: string) =>
...
jest.mock('react-native-libsodium', () => {
  mockSodium = {
    ...
    crypto_generichash: jest.fn((len: number, input: Uint8Array) => {
      ...
      for (let i = 0; i < len; i++)
        out[i] = (input[i % input.length] + i) % 256;
      return out;
    }),
    crypto_sign_keypair: jest.fn(() => ({
      privateKey: u8(1, 2, 3, 4),
      publicKey: u8(5, 6, 7, 8),
    })),
    crypto_box_keypair: jest.fn(() => ({
      privateKey: u8(9, 10, 11, 12),
      publicKey: u8(13, 14, 15, 16),
    })),
    ...
    randombytes_buf: jest.fn((n: number) => {
      ...
      // Byte truncation is intentional in this mock.
      for (let i = 0; i < n; i++) out[i] = (i + 1) % 256;
      return out;
    }),
    ...
    crypto_secretbox_open_easy: jest.fn((cipher: Uint8Array) =>
      cipher.slice(0, cipher.length - 2),
    ),
  };
  return {
    __esModule: true,
...
describe('EncryptedDMService', () => {
  ...
  it('rejects invalid bundle signatures and version', () => {
    ...
    expect(() => encryptedDMService.verifyBundle(validBundle as any)).toThrow(
      'Bad signature',
    );
    expect(() =>
      encryptedDMService.verifyBundle({ ...validBundle, v: 2 } as any),
  ...
  it('parses and exports external payloads', async () => {
    ...
    const fpPayload =
      await encryptedDMService.exportFingerprintPayload('alice');
    const parsedFp = encryptedDMService.parseExternalPayload(fpPayload);
    expect(parsedFp.type).toBe('encdm-fingerprint');

    expect(() =>
      encryptedDMService.parseExternalPayload(JSON.stringify({ v: 2 })),
    ).toThrow('Invalid payload');
    expect(() =>
      encryptedDMService.parseExternalPayload(
        JSON.stringify({ v: 1, type: 'x' }),
      ),
  ...
  it('acceptExternalBundle enforces allowReplace policy', async () => {
    ...
    await expect(
      encryptedDMService.acceptExternalBundle('alice', changed as any, false),
    ).resolves.toBeUndefined();
    await expect(
      encryptedDMService.acceptExternalBundle('alice', changed as any, true),
    ).resolves.toBeUndefined();
  });

  it('waits for incoming bundle and times out when missing', async () => {
    await expect(
      encryptedDMService.awaitBundleForNick('bob', 5),
  ...
  it('handles incoming bundle with same/new/changed and resolves waiter', async () => {
    ...
    await encryptedDMService.handleIncomingBundle(
      'alice',
  ...
  it('handles key offer/accept/reject/acceptance branches', async () => {
    ...
    expect(
      await encryptedDMService.handleKeyOffer(
        'alice',
        JSON.stringify(validBundle),
      ),
    ).toBe(true);
    ...
    await expect(encryptedDMService.acceptKeyOffer('alice')).rejects.toThrow(
      'No pending offer',
    );
    ...
    const acceptance = await encryptedDMService.handleKeyAcceptance(
      'alice',
      JSON.stringify(changed),
    );
    ...
    expect(
      await encryptedDMService.handleKeyAcceptance('alice', '{bad'),
  ...
  it('stores and reads network-aware keys and verification flags', async () => {
    await encryptedDMService.storeBundleForNetwork(
      'net1',
      'alice',
      validBundle as any,
    );
    expect(
      await encryptedDMService.getBundleForNetwork('net1', 'alice'),
    ).toEqual(validBundle);
    expect(
      await encryptedDMService.isEncryptedForNetwork('net1', 'alice'),
    ).toBe(true);
    expect(
      await encryptedDMService.getBundleFingerprintForNetwork('net1', 'alice'),
    ).toBeTruthy();

    const s1 = await encryptedDMService.getVerificationStatusForNetwork(
      'net1',
      'alice',
    );
    expect(s1.verified).toBe(false);
    await encryptedDMService.setVerifiedForNetwork('net1', 'alice', true);
    const s2 = await encryptedDMService.getVerificationStatusForNetwork(
      'net1',
      'alice',
    );
  ...
  it('network-aware offer/accept/reject/acceptance flows', async () => {
    ...
    expect(
      await encryptedDMService.handleKeyOfferForNetwork(
        'net1',
        'alice',
        JSON.stringify(validBundle),
      ),
    ).toBe(true);
    const selfBundle = await encryptedDMService.acceptKeyOfferForNetwork(
      'net1',
      'alice',
    );
    ...
    const result = await encryptedDMService.handleKeyAcceptanceForNetwork(
      'net1',
      'alice',
      JSON.stringify(changed),
    );
    ...
    expect(cb).not.toHaveBeenCalled();
    expect(
      await encryptedDMService.handleKeyAcceptanceForNetwork(
        'net1',
        'alice',
        'bad-json',
      ),
  ...
  it('network-aware incoming bundle and external accept policy', async () => {
    await encryptedDMService.handleIncomingBundleForNetwork(
      'net1',
      'alice',
      JSON.stringify(validBundle),
    );
  ...
  it('encrypts/decrypts legacy with aad fallback and validation errors', async () => {
    ...
    mockSodium.crypto_aead_xchacha20poly1305_ietf_decrypt
      ...
      .mockImplementationOnce((_n: any, cipher: Uint8Array) =>
        cipher.slice(0, cipher.length - 1),
      );
    const plainFallback = await encryptedDMService.decrypt(enc as any, 'alice');
    expect(plainFallback).toBe('hello');

    await expect(
      encryptedDMService.decrypt({ ...enc, v: 2 } as any, 'alice'),
    ).rejects.toThrow('Invalid version');
    await expect(
      encryptedDMService.decrypt(
        { ...enc, from: b64(u8(1, 2, 3)) } as any,
        'alice',
      ),
  ...
  it('encrypts/decrypts network-aware and derives message key', async () => {
    ...
    const key = await encryptedDMService.getMessageKeyForNetwork(
      'net1',
      'alice',
    );
    expect(key).toBeInstanceOf(Uint8Array);

    const enc = await encryptedDMService.encryptForNetwork(
      'hello-net',
      'net1',
      'alice',
    );
    const plain = await encryptedDMService.decryptForNetwork(
      enc as any,
      'net1',
      'alice',
    );
    ...
    await expect(
      encryptedDMService.getMessageKeyForNetwork('net1', 'missing'),
    ).rejects.toThrow('No bundle');
    await expect(
      encryptedDMService.decryptForNetwork(
        { ...enc, v: 2 } as any,
        'net1',
        'alice',
      ),
  ...
  it('lists, copies, moves, deletes and migrates keys', async () => {
    ...
    expect(list.map(k => `${k.network}:${k.nick}`)).toEqual([
      'netA:alice',
      'netB:bob',
    ]);
    ...
    expect(
      await encryptedDMService.isEncryptedForNetwork('netC', 'alice'),
    ).toBe(true);
    await encryptedDMService.moveBundleToNetwork('netC', 'netD', 'alice');
    expect(
      await encryptedDMService.isEncryptedForNetwork('netD', 'alice'),
    ).toBe(true);
    ...
    expect(
      await encryptedDMService.isEncryptedForNetwork('netM', 'legacy'),
    ).toBe(true);
  });

  it('throws copy error when source key missing', async () => {
    await expect(
      encryptedDMService.copyBundleToNetwork('a', 'b', 'nobody'),
  ...
  it('handles import backup errors (format/version/decrypt)', async () => {
    ...
    await expect(
      encryptedDMService.importKeyBackup('bad', 'pass'),
    ...
    const nonce = new Uint8Array(mockSodium.crypto_secretbox_NONCEBYTES).fill(
      2,
    );
    ...
    await expect(encryptedDMService.importKeyBackup('x', 'p')).rejects.toThrow(
  ...
  it('covers additional edge branches for formatting, same-bundle paths, and network waiters', async () => {
    ...
    expect(await encryptedDMService.getVerificationStatus('unknown')).toEqual({
      fingerprint: null,
      verified: false,
    });
    expect(
    ...
    const acceptanceStored = await encryptedDMService.handleKeyAcceptance(
      'alice',
      JSON.stringify(validBundle),
    );
  ...
  it('covers legacy/network changed-offer guard branches', async () => {
    ...
    await expect(
      encryptedDMService.acceptKeyOffer('alice'),
    ).resolves.toBeTruthy();
    ...
    await expect(
      encryptedDMService.acceptKeyOfferForNetwork('net1', 'alice'),
    ).resolves.toBeTruthy();
  });
  ...
  it('covers export/list/import warning branches for malformed entries', async () => {
    ...
    mockSodium.from_base64.mockReturnValueOnce(combined);
    mockSodium.crypto_secretbox_open_easy.mockReturnValueOnce(
      new Uint8Array(Buffer.from(backupPayload)),
    );
    const imported = await encryptedDMService.importKeyBackup(
      'payload',
      'pass',
    );
  ...
  it('handles malformed JSON inputs in key handlers', async () => {
    expect(await encryptedDMService.handleKeyOffer('alice', 'not-json')).toBe(
      false,
    );
    expect(
      await encryptedDMService.handleKeyOfferForNetwork(
